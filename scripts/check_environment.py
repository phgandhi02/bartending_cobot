"""
File Description: A script(Python or Bash) that verifies the host machine is correctly configured for ROS2 before any nodes are launched.

This catches "it works on my machine" errors early before deployment.
...
try:  # Wrap ROS imports so we can exit with a clear error message instead of a stack trace
    ...
    from std_msgs.msg import (
        String,
    )  # Imports a simple ROS 2 message type used for our loopback test
except ImportError as exc:  # If ROS Python packages are missing, we handle it gracefully
    print(
        "ERROR: Could not import rclpy/std_msgs. Did you source ROS 2 (source /opt/ros/humble/setup.bash)?"
    )  # Explain the likely root cause in simple terms
    print(f"ImportError details: {exc}")  # Print the underlying import error to help debugging
    sys.exit(
        2
    )  # Exit with a unique non-zero code to signal a ROS Python dependency/import problem

# Check for The ROS_DOMAIN_ID -> if not found, default to "0"
# We use "0" as a backup value in .get() because environments variables are always strings.
ros_domain_id = os.environ.get(
    "ROS_DOMAIN_ID", "0"
)  # Everything inside the dictionary is stored as a String
print(
    f"The ROS DOMAIN ID is: {ros_domain_id}"
)  # The 'f' tells Python to swap the {variable} inside the braces for its actual value
...
while (len(received_list) == 0) and ((time.time() - start_time) < timeout_sec):
    ...
    )  # Spin in small chunks to process DDS discovery and callbacks reliably
    ...
    if (time.time() - last_publish_time) >= republish_interval_sec:
        publisher.publish(
            msg
        )  # Re-send the same message object to increase reliability during discovery
...
rmw_implementation = os.environ.get(
    "RMW_IMPLEMENTATION"
)  # Read the requested RMW from the environment (None means "use the default")
known_rmw = {
    "rmw_fastrtps_cpp",
    "rmw_cyclonedds_cpp",
    "rmw_connextdds",
}  # Define a small known-good set of common ROS 2 RMW implementations
...
if rmw_implementation is None:  # If unset, ROS 2 will use the system default RMW implementation
    ...
    if (
        rmw_implementation not in known_rmw
    ):  # If the value is unexpected, this could indicate a misconfiguration
        print(
            f"Error: Unknown RMW_IMPLEMENTATION='{rmw_implementation}'"
        )  # Explain that the env var value is not recognized
        print(
            f"Known values include: {sorted(known_rmw)}"
        )  # Show the user acceptable values to reduce guesswork
        sys.exit(
            4
        )  # Exit with a unique non-zero code to signal middleware configuration inconsistency
    print(
        f"The RMW Implementation is: {rmw_implementation}"
    )  # Confirm the middleware choice when it is explicitly set and recognized

# Run a command and capture output
try:
    result = subprocess.run(["apt", "list", "--installed", "gz-harmonic"], check=True)
except subprocess.CalledProcessError as e:
    print("gz-harmonic not installed")
...
try:
    result = subprocess.run(
        ["apt", "list", "--installed", "ros-jazzy-joint-trajectory-controller"], check=True
    )
except subprocess.CalledProcessError as e:
    print("necessary ros2 controllers not installed")
    print("ros-jazzy-joint-trajectory-controller not installed")
    print(f"Command failed with return code: {e.returncode}")
    print(f"Command: {e.cmd}")
    print(f"Error output: {e.output}")
